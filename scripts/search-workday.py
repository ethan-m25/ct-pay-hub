#!/usr/bin/env python3
"""
ct-pay-hub/scripts/search-workday.py
Workday API scraper — Connecticut edition.

CT Equal Pay Act (effective Oct 1, 2021)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Connecticut employers
  2. CXS JSON API   — paginate all jobs per employer, filter Connecticut (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/ct-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/ct-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/ct-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Connecticut"
...
SEED_TENANTS = [
    ...
    ("thehartford.wd5.myworkdayjobs.com", "thehartford", "External", "The Hartford"),
    ("synchrony.wd1.myworkdayjobs.com", "synchrony", "External", "Synchrony Financial"),
    ("cigna.wd5.myworkdayjobs.com", "cigna", "Cigna", "Cigna"),
    ("gartner.wd5.myworkdayjobs.com", "gartner", "Gartner_Careers", "Gartner"),
    ...
    ("travelers.wd5.myworkdayjobs.com", "travelers", "External", "Travelers"),
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Connecticut" salary 2026',
    'site:myworkdayjobs.com "Hartford" OR "Stamford" job salary 2026',
    'site:myworkdayjobs.com "Bridgeport" OR "New Haven" OR "Greenwich" salary 2026',
    'site:myworkdayjobs.com Connecticut defense OR insurance OR finance salary 2026',
    'site:myworkdayjobs.com "Norwalk" OR "Waterbury" OR "Danbury" CT salary 2026',
]
...
def is_connecticut(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Connecticut.
...
_BRAND_MAP = [
    ...
    (re.compile(r'ssga trust|state street global advisors', re.IGNORECASE), "State Street"),
    (re.compile(r'travelers indemnity', re.IGNORECASE),     "Travelers"),
]
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_connecticut(p.get("locationsText", ""), p.get("externalPath", "")):
                    ct_jobs.append(p)
            offset += limit
        ...
        log(f"  Connecticut jobs: {len(ct_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(ct_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_connecticut(location, external_path):
            continue
