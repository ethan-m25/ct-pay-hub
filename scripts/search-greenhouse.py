#!/usr/bin/env python3
"""
ct-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — Connecticut edition.
...
Run: python3 ~/ct-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/ct-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/ct-pay-hub/scripts/.greenhouse.lock")
...
SEED_SLUGS = [
    ...
    ("relaygraduateschoolofeducation", "Relay Graduate School"),
    ("careaccess", "Care Access"),
    ("synchrony", None),
    ("cigna", None),
    ("hartfordfunds", "Hartford Funds"),
...
def is_ct_job(title: str, location: str, content: str) -> bool:
...
def fetch_company_jobs(slug: str, company_name_override=None):
    ...
    for j in jobs_raw:
        ...
        if not is_ct_job(title, location, content_text):
            continue
...
def main():
    ...
    log("=== CT Greenhouse scraper started ===")
    ...
    log(f"=== Done. {new_count} new CT jobs written to {OUTPUT_FILE} ===")
