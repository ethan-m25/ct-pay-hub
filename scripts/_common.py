#!/usr/bin/env python3
"""
ct-pay-hub/scripts/_common.py
Shared utilities for all Connecticut Pay Hub search scripts.
Connecticut Equal Pay Act (PA 21-30) — effective Oct 1, 2021 — employers must provide salary range upon request AND in job postings (for positions advertised publicly)
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"ct-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/ct-pay-hub/data/jobs.json")
CURRENCY    = "USD"
REGION      = "CT"
...
REGION_TERMS = ['connecticut', 'hartford', 'new haven', 'stamford', 'bridgeport', 'waterbury', 'norwalk', 'danbury', 'greenwich', 'westport', 'fairfield', 'new london', 'middletown, ct', 'meriden', 'new britain', 'west hartford', 'enfield, ct', 'shelton', 'milford, ct', 'trumbull', 'glastonbury', ', ct', 'ct,', 'fairfield county', 'new haven county']

_NON_REGION_LOC_TERMS = ['toronto', 'vancouver', 'ontario, canada', 'british columbia', 'canada,', 'washington, dc', 'washington dc', 'denver, co', 'boulder, co', 'seattle, wa', 'washington state', 'new york city', 'new york, ny', 'san francisco, ca', 'los angeles, ca', 'chicago, il', 'boston, ma', 'cambridge, ma', 'newark, nj', 'jersey city, nj', 'baltimore, md', 'minneapolis, mn']
...
EXTRACT_PROMPT = """\
Extract ONE Connecticut job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid Connecticut job with explicit USD salary range is found:
{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"Connecticut location","source_url":"{url}","posted":"YYYY-MM-DD"}
...
- location must be in Connecticut
